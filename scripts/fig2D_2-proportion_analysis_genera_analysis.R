...
melted_prev <- merge(melted_prev_sb,Dat_rar_sb$Map, by = "DADA2_Header")
...
usable_samples_sb <- df_sum_03$ASV %>% unique %>% as.character #unique ASV 102
...
merged4_sb <- merged3_sb %>%
  subset(RA_zscore > 0.3) %>% droplevels
...
melted_prev_sd <- merge(melted_prev_sd,Dat_rar_sd$Map, by = "DADA2_Header")
...
######select all the ASVs with a RA-zscore > 0.3
...
######select the prevalence ASV
usable_samples_sd <- df_sum_03_sd$ASV %>% unique %>% as.character #unique ASV 3885
...
melted_prev_t <- merge(melted_prev_t,Dat_rar_t$Map, by = "DADA2_Header")
...
######select the prevalence ASV
usable_samples_t <- df_sum_03_t$ASV %>% unique %>% as.character #unique ASV 3885
...
prop_l1 <- prop.test(x = c(length(intersect_sb_sd_05),
                           length(intersect_sb_t_05)), 
                     n = c(length(ids_sb_l1_05)+length(ids_sd_l1_05), 
                           length(ids_sb_l1_05)+length(ids_t_l1_05)),
          alternative = "greater")
