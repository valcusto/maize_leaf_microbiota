...
######open the defence genes dataset
df_defence <- read.csv('../cleandata/defense_genes_in_CR468_CR8C.csv')

######remove CR8C
df_defense_cr468 <- df_defence %>%
  subset(ClusterRows != 'CR8C') %>% droplevels

######select only defense genes
df_defense_cr468 <- df_defense_cr468 %>%
  subset(category == 'defense') %>% droplevels

######remove the outlier Zm00001eb032600
df_defense_cr468 <- df_defense_cr468 %>%
  subset(IdRows != 'Zm00001eb032600') %>% droplevels
...
######match the dataset
df_defense_unique <- df_defense_cr468 %>%
  dplyr::filter(.data = .,IdRows %in% toselect) %>% droplevels

######select the genes significant genes in L3
toselect_defense <- df_defense_unique$IdRows %>% as.character %>% unique
...
melted_sub_all <- melted_all %>%
  dplyr::filter(.data = .,gene_id %in% toselect_defense) %>% droplevels

######add the cluster information
melted_sub_all$ClusterRows <- match(melted_sub_all$gene_id,df_defense_unique$IdRows) %>%
  df_defense_unique$ClusterRows[.]
...
p_cr4_final <- p_cr4 +
  ...
  geom_line(data = tibble(x=c(5,6), y=c(1.9, 1.9)), aes(x=x, y=y),
            inherit.aes = FALSE) +
  geom_text(data = tibble(x=1.5, y=2), aes(x=x, y=y, label = 'NS'),
            inherit.aes = FALSE) +
...
######save figures
oh.save.pdf(p = composition,outname = "figS4L_gene_expression_only_defence_per_cluster_all_leaves_ttest_final.pdf",
