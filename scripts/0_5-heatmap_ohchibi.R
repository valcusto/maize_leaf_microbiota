...
#' @examples
#' chibi.heatmap()
chibi.heatmap_l3<- function(Tab = NULL,df_border = NULL,df_tile_col = NULL,
                         ...
                         range_fill_heatmap = c(-1,1),size_strip_text_row = 9,
                         size_strip_text_col = 0,
                         size_axis_text_col = 10,size_axis_text_row = 0,
                         axis_ticks_row = F,
                         size_legend_text = 10, size_legend_title = 11,
                         size_border_tile = 0.5, width_border_tile = 0.85,
                         height_border_tile = 0.85,
                         palette_border = c("black"),legend_proportion_size = 0.75,
                         size_dendrogram = 0.3,panel_border_heatmap = 0.3,
                         panel_spacing = 0,
                         font_family = "Helvetica",font_face = "plain",
                         legend_position = "right",
                         tree_scale_factor_cols = c(0.005,0.005),
                         tree_scale_factor_rows = c(0.001,0.001),
                         egg_heights = c(0.2,1),egg_widths = c(0.1,1),
                         mtheme = NULL){
  ...
  if(is.null(df_border)){
    p <- ggplot(data = melted_sub,mapping = aes(IdCols,IdRows)) +
      geom_tile(aes(fill = value),color = color_tile) +
    ...
    p <- ggplot(data = melted_sub,mapping = aes(IdCols,IdRows)) +
      ...
      scale_y_discrete(expand  = c(0,0)) +
      scale_fill_paletteer_c(palette_heatmap,
                             limits = range_fill_heatmap,oob = squish,
                             name = "Standardized\nexpression") +
      scale_color_manual(values = palette_border,na.value = "#00000000")
  }
  
  if(geom_text != FALSE){
    p <- p + geom_text(aes(label = round(value,round_geom_text)),
                       size = size_geom_text)
  }
...
chibi.heatmap_l3_nb<- function(Tab = NULL,df_border = NULL,df_tile_col = NULL,
                            dist_method_rows = "pearson",
                            dist_method_cols = "euclidean",
                            hclust_method_rows = "ward.D",
                            hclust_method_cols = "ward.D",
  ...
  if(is.null(df_border)){
    p <- ggplot(subset(data = melted_sub,IdCols %in% c("L3_FullSynCom", 
                                                       "L3_NB")),
                mapping = aes(IdCols,IdRows)) +
      geom_tile(aes(fill = value),color = color_tile) +
      facet_grid(ClusterRows~.,scales = "free",space = "free") +
      theme_ohchibi(font_family = font_family
      ) +
    ...
    colnames(melted_sub)[ncol(melted_sub)] <- "Border"
    p <- ggplot(subset(data = melted_sub,IdCols %in% c("L3_FullSynCom", 
                                                       "L3_NB")),mapping = aes(IdCols,IdRows)) +
      geom_raster(aes(fill = value),color = "#00000000") +
      geom_tile(aes(color = Border),fill = '#00000000',
                size = size_border_tile,width = width_border_tile,height = height_border_tile) +
      facet_grid(ClusterRows~.,scales = "free",space = "free") +
...
#######################Leaf 4
chibi.heatmap_l4<- function(Tab = NULL,df_border = NULL,df_tile_col = NULL,
                            dist_method_rows = "pearson",
                            dist_method_cols = "euclidean",
                            hclust_method_rows = "ward.D",
                            hclust_method_cols = "ward.D",
                            ...
                            tree_scale_factor_cols = c(0.005,0.005),
                            tree_scale_factor_rows = c(0.001,0.001),
                            egg_heights = c(0.2,1),egg_widths = c(0.1,1),mtheme = NULL){
  if(dist_method_rows == "pearson" | dist_method_rows == "spearman"){
    dist_rows <- as.dist(1-cor(Tab %>% t,method = dist_method_rows))
...
#######################Leaf 5
chibi.heatmap_l5<- function(Tab = NULL,df_border = NULL,df_tile_col = NULL,
                            dist_method_rows = "pearson",
                            dist_method_cols = "euclidean",
                            hclust_method_rows = "ward.D",
                            hclust_method_cols = "ward.D",
