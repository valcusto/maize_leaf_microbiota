...
melted <- merge(melted, Dat$Map, by='UiD')

######create a group column
melted$group <- paste(melted$Nutrient, melted$NumLeaf, melted$Treatment,
                      sep = '_')
...
melted_sum$Treatment <- sapply(strsplit(as.character(melted_sum$group),
                                      '_'), `[`, 3)
...
######change the name in the Nutrient
melted_sum$Treatment[which(melted_sum$Treatment == 'Full syncom')] <- '+SynCom'
melted_sum$Treatment[which(melted_sum$Treatment == 'No Bacteria')] <- 'NB'

###create a group with the treatment and nutrient
melted_sum$group <- paste(melted_sum$Treatment, melted_sum$Nutrient, sep='_')

######organize the group
melted_sum$group <- melted_sum$group %>%
  factor(levels = c('NB_Full Hoagland', '+SynCom_Full Hoagland',
                    'NB_1/4 Hoagland','+SynCom_1/4 Hoagland'))

######plot 
p_heatmap <- ggplot(data =melted_sum,aes(x = group,y = ion)) +
...
oh.save.pdf(p = composition,outname = "figS3P_Heatmap_leaf_ionome_with_without_bacteria.pdf",
