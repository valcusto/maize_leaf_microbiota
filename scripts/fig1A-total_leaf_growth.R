...
df_mean <- aggregate(leaf_lenght_cm ~ num_pot+soil+experiment,
                     df_sub, sum)
...
#perform levene test
leveneTest(leaf_lenght_cm ~ soil, data = df_mean)
#At least one soil has a variance that is not equal to the rest
#3.check the normality of the data
qqnorm(df_mean$leaf_lenght_cm)
qqline(df_mean$leaf_lenght_cm, col='red')

##Data exploration
ggplot(data = df_mean,mapping = aes(x = soil,y = leaf_lenght_cm)) +
  geom_boxplot() + 
  geom_point()

######apply kruskal-wallis test
m1 <- kruskal.test(leaf_lenght_cm ~ soil,
                   data = df_mean)

m1$p.value

DT = dunnTest(leaf_lenght_cm ~ soil,data=df_mean,method="bh")
...
msoil <- df_mean$soil %>% unique
###Empty dataset
Res_ci <- NULL

df_sb <- df_mean %>% subset(soil == 'Sutton Bonington') %>% droplevels
df_mean_sb <- mean(df_sb$leaf_lenght_cm) %>% as.data.frame
df_mean_sb$soil <- 'SuttonBonington'
colnames(df_mean_sb)[1] <- 'mean'
df_sd <- df_mean %>% subset(soil == 'Sao Domingos') %>% droplevels
df_mean_sd <- mean(df_sd$leaf_lenght_cm) %>% as.data.frame
df_mean_sd$soil <- 'SaoDomingos'
colnames(df_mean_sd)[1] <- 'mean'
df_t <- df_mean %>% subset(soil == 'Tarrafal') %>% droplevels
df_mean_t <- mean(df_t$leaf_lenght_cm) %>% as.data.frame
...
######calculate the ci
results_sb <- boot(data=df_sb$leaf_lenght_cm, statistic=Bmean, R=1000)
...
results_sd <- boot(data=df_sd$leaf_lenght_cm, statistic=Bmean, R=1000)
...
results_t <- boot(data=df_t$leaf_lenght_cm, statistic=Bmean, R=1000)
...
df$soil <- df$soil %>%
  factor(levels = c('Sao Domingos', 'Tarrafal', 'Sutton Bonington'))
df_mean$soil <- df_mean$soil %>%
  factor(levels = c('Sao Domingos', 'Tarrafal', 'Sutton Bonington'))

######plot the result
p_leaf <- ggplot(data=df_mean, aes(x=soil, y=leaf_lenght_cm, 
                                     shape=soil, color=soil))+
  geom_point(shape=21, size=4, alpha=0.3)+
  geom_pointrange(data=df, aes(y=mean, ymin=lower, ymax=upper, 
                               shape=soil), 
                  size=1.5)+
  geom_text(data = df, aes(x = soil,y = 240,
                           label = Letter),
...
######save figures
oh.save.pdf(p = p_leaf,outname = "fig1C_total_leaf_lenght.pdf",
